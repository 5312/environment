  ...
  >
    <div class="ele-setting-wrapper">
      ...
      <div class="ele-setting-theme">
        <el-tooltip
          v-for="(item, index) in sideStyles"
          :key="index"
          :content="item.name"
          placement="top"
        >
          <div
            :class="[
              item.style,
              { active: $store.state.theme.sideStyle === item.value },
            ]"
            @click="setSideStyle(item.value)"
          ></div>
        </el-tooltip>
      </div>
      <!-- 顶栏风格设置 -->
      <div class="ele-setting-theme">
        <el-tooltip
          v-for="(item, index) in headStyles"
          :key="index"
          :content="item.name"
          placement="top"
        >
          <div
            :class="[
              item.style || 'ele-head-light',
              { active: $store.state.theme.headStyle === item.value },
            ]"
            @click="setHeadStyle(item.value)"
          ></div>
        </el-tooltip>
      </div>
      <!-- 主题颜色设置 -->
        ...
        >
          <div
            :class="[
              ...
              { active: $store.state.theme.theme === item.value },
            ]"
      ...
      <div class="ele-setting-theme">
        <el-tooltip content="左侧菜单布局" placement="top">
          <div
            :class="[
              'ele-side-dark',
              { active: $store.state.theme.layoutStyle === 0 },
            ]"
            @click="setLayout(0)"
          ></div>
        </el-tooltip>
        <el-tooltip content="顶部菜单布局" placement="top">
          <div
            :class="[
              'ele-head-dark',
              { active: $store.state.theme.layoutStyle === 1 },
            ]"
            @click="setLayout(1)"
          ></div>
        </el-tooltip>
        <el-tooltip content="混合菜单布局" placement="top">
          <div
            :class="[
              'ele-layout-mix',
              { active: $store.state.theme.layoutStyle === 2 },
            ]"
            @click="setLayout(2)"
          ></div>
        </el-tooltip>
      ...
      <!-- <div class="ele-setting-item">
        <div class="setting-item-title">侧栏展开</div>
        <div class="setting-item-control">
...
export default {
  ...
  data () {
    return {
      ...
      /* 主题色 */
      themeColors: [
        { name: "暗黑", value: "dark", color: "#000000" },
        { name: "科技蓝", value: null, color: "#020533e8" },
  ...
  mounted () {
    if (this.$store.state.theme.theme) {
      this.setTheme(this.$store.state.theme.theme, true)
    } // 恢复上次设置主题
  },
  methods: {
    /* 切换侧边栏风格 */
    ...
    setLayout (style) {
      ...
      if (style === 2) {
        this.$store.dispatch("theme/setNavActive", 0)
      } else {
        this.$store.dispatch("theme/setNavActive")
      }
    }
  },
  computed: {
