<!-- 侧边栏 -->
<template>
  <el-aside class="my-top ele-admin-sidebar ele-scrollbar-hide">
    <el-menu
      :collapse="collapse"
      :default-active="active"
      @select="onMenuSelect"
      :collapse-transition="false"
      :class="{ 'ele-menu-dark': theme.sideStyle === 1 }"
      :unique-opened="theme.sideUniqueOpen"
    >
      <template v-for="(menu, index) in menus">
        <ele-sidebar-item
          v-if="!menu.meta || !menu.meta.hide"
          :item="menu"
          :path="getPath(menu.path) || index"
          :key="menu.path || index"
          :pop-class="theme.sideStyle === 1 ? 'ele-menu-dark' : ''"
        />
      </template>
    </el-menu>
  </el-aside>
...
export default {
  ...
  computed: {
    ...
    menus () {
      ...
      let arr = menusJson.filter(d => {
        if (d.path == this.layout.navIndex) {
          return true
        } else {
          false
        }
      })
      return arr.length > 0 ? arr[0].children : arr
    },
    /* 是否折叠menu */
    collapse () {
  ...
  methods: {
    ...
    onMenuSelect (index) {
      ...
      if ((parent && parent.redirect === path && parent.path === index) || path === index) {
        this.refresh()
      } else {
    ...
    /* 获取路径, 多系统模式嵌套路由加父路径 */
    getPath (url) {
      if (!(this.theme.layoutStyle === 2 && this.theme.screenWidth >= 768)) {
        return url || ""
      }
