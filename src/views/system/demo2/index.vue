  ...
  <div class="ele-body">
    <el-card shadow="never">
      ...
      <div class="ele-table-tool ele-table-tool-default">
        <el-button @click="showEdit=true" type="primary" icon="el-icon-plus" class="ele-btn-icon" size="small" v-if="permission.includes('sys:demo2:add')">添加
        </el-button>
        <el-button @click="remove()" type="danger" icon="el-icon-delete" class="ele-btn-icon" size="small" v-if="permission.includes('sys:demo2:dall')">批量删除
        </el-button>
      </div>
      <!-- 数据表格 -->
      <ele-data-table ref="table" :config="table" :choose.sync="choose" height="calc(100vh - 315px)" highlight-current-row>
        <template slot-scope="{index}">
          ...
          <el-table-column label="操作" width="130px" align="center" :resizable="false"  fixed="right">
            <template slot-scope="{row}">
              <el-link @click="edit(row)" icon="el-icon-edit" type="primary" :underline="false" v-if="permission.includes('sys:demo2:edit')">修改</el-link>
              <el-popconfirm title="确定要删除此演示吗？" @confirm="remove(row)" class="ele-action">
                <el-link slot="reference" icon="el-icon-delete" type="danger" :underline="false" v-if="permission.includes('sys:demo2:delete')">删除</el-link>
              </el-popconfirm>
            </template>
          </el-table-column>
        </template>
...
export default {
  ...
  data() {
    return {
      table: {url: "/demo2/index", where: {}},  // 表格配置
      choose: [],  // 表格选中数据
  ...
  methods: {
     ...
     */
    edit(row) {
      this.$http.get("/demo2/info?id=" + row.id).then(res => {
        if (res.data.code === 0) {
    ...
    save() {
      this.$refs["editForm"].validate((valid) => {
        if (valid) {
          ...
          this.$http.post("/demo2/edit", this.editForm).then(res => {
            loading.close();
            if (res.data.code === 0) {
              this.showEdit = false;
    ...
    remove(row) {
      ...
      if (!row) {  // 批量删除
        ...
        this.$confirm("确定要删除选中的演示吗?", "提示", {type: "warning"}).then(() => {
          const loading = this.$loading({lock: true});
          this.$http.post("/demo2/delete", {id: ids}).then(res => {
            loading.close();
            if (res.data.code === 0) {
              this.$message({type: "success", message: res.data.msg});
              this.$refs.table.reload();
        ...
        const loading = this.$loading({lock: true});
        this.$http.post("/demo2/delete", {id: row.id}).then(res => {
          loading.close();
          if (res.data.code === 0) {
            this.$message({type: "success", message: res.data.msg});
            this.$refs.table.reload();
    ...
    status(row) {
      ...
      this.$http.post("/demo2/status", params).then(res => {
        loading.close();
        if (res.data.code === 0) {
          this.$message({type: "success", message: res.data.msg});
        } else {
    ...
    setIsVip(row) {
      const loading = this.$loading({lock: true});
      let params = Object.assign({}, row);
      this.$http.post("/demo2/setIsVip", params).then(res => {
        loading.close();
        if (res.data.code === 0) {
          this.$message({type: "success", message: res.data.msg});
        } else {
