  ...
  methods: {
    ...
    remove(row) {
      if (!row) {
        ...
        this.$confirm("确定要删除选中的角色吗?", "提示", { type: "warning" })
          .then(() => {
            const loading = this.$loading({ lock: true });
            this.$http
              .post("/role/delete", { id: ids })
              .then((res) => {
                loading.close();
                if (res.data.code === 0) {
                  this.$message({ type: "success", message: res.data.msg });
    ...
    auth(row) {
      ...
      this.$http
        .get("/role/getPermissionList?role_id=" + row.id)
        .then((res) => {
          loading.close();
          this.showAuth = true;
          if (res.data.code === 0) {
    ...
    saveAuth() {
      const loading = this.$loading({ lock: true });
      let ids = this.$refs.authTree
        .getCheckedKeys()
        .concat(this.$refs.authTree.getHalfCheckedKeys());
      this.$http
        .post("/role/savePermission?role_id=" + this.editForm.id, {menu_ids:ids})
        .then((res) => {
          loading.close();
          if (res.data.code === 0) {
            this.showAuth = false;
