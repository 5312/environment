    ...
    <el-card shadow="never">
      ...
      >
        ...
        >
          <template slot-scope="{ row }">
            <ele-dot
              :type="['success', 'none'][row.hide]"
              :ripple="row.hide === 0"
              :text="['显示', '隐藏'][row.hide]"
            />
          </template>
        </el-table-column>
        <el-table-column prop="method" label="方法" sortable min-width="100">
          <template slot-scope="{ row }">
            <el-tag
              :type="['', 'success', 'warning', 'danger'][row.method]"
              size="mini"
              >{{ ["GET", "POST", "PUT", "DELETE"][row.method] }}</el-tag
            >
          </template>
        </el-table-column>
        ...
        <el-table-column label="类型" width="60px" align="center">
          <template slot-scope="{ row }">
            <el-tag :type="['primary', 'success'][row.type]" size="mini">{{
              ["菜单", "按钮"][row.type]
            }}</el-tag>
          </template>
        </el-table-column>
...
export default {
  ...
  methods: {
    ...
    load(tree, treeNode, resolve) {
      ...
      this.$http
        .get("/menu/index", { params: this.where })
        .then((res) => {
          if (res.data.code === 0) {
    ...
    edit(row) {
      this.$http
        .get("/menu/info?id=" + row.id)
        .then((res) => {
          if (res.data.code === 0) {
            let arr = res.data.data;
            if (res.data.data.checkedList instanceof Array) {
              console.log("empty");
            } else {
              let check = res.data.data.checkedList;
              let checked = [];
