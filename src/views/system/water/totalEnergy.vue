    ...
    <div v-if="index == 3" key="123132" class="type-zhilu">
      <el-row :gutter="10" v-loading="topall">
        ...
        <el-col :span="12">
          <el-card shadow="never">
            <div class="title">
              水<i class="el-icon-heavy-rain gree"> </i>
              <el-button style="float: right; padding: 3px 0" type="text">
                <!-- 单位: 吨 -->
                <span v-if="twobtn == '1'"> 单位: 吨</span>
                <span v-else>单位:元</span>
              </el-button>
            </div>
            <el-table :show-header="false" :data="shui" style="width: 100%">
        ...
        <el-col :span="16">
          <el-card shadow="never">
            ...
            <el-table :data="tableData" style="width: 100%" height="250">
              ...
              <el-table-column prop="shui" label="能源量"> </el-table-column>
...
export default {
  ...
  data () {
    return {
      ...
      tableData: [],
      /****************/
      electricityData: [], // [12, 32, 23, 2, 32, 3, 2, 3], // 用水占比数据
  ...
  computed: {
    ...
    monthAllTopCardData () {
      if (this.month == 1) {
        ...
        return this.shui.length != 0 ? this.shui[1].b : "0"
      }
    },
    yesTadaySame () {
      if (this.month == 1) {
        ...
        return this.shui.length != 0
          ? [this.shui[1].s, this.shui[1].z]
          : [0, 0]
      }
    },
    mei () {
    ...
    shui () {
      if (this.coutsIndex != "") {
        let retArr = []
        let array = this.coutsIndex.shui
        retArr.push(array.dayc)
        retArr.push(array.mothc)
        retArr.push(array.yearc)
        return retArr
  ...
  methods: {
    ...
    async todayShui () {
      this.todayLoad = true
      const result_c = await this.$http.get("/energy/getQsDay", {
        params: {
          dept_id: this.dept,
          road_id: 0,
          cate_id: 2,
      ...
      let tDay = myDate.getDate() - 1

      tMonth = util.doHandleZero(tMonth + 1)
      tDay = util.doHandleZero(tDay)

      const result_y = await this.$http.post("/energy/getQsDay", {
        dept_id: this.dept,
        road_id: 0,
        cate_id: 2,
        types: this.today,
        dates: tYear + "-" + tMonth + "-" + tDay,
      })
      this.todayLoad = false
      this.today_branch_threeData.dayc = result_c.data.data.b
      this.today_branch_threeData.ydayc = result_y.data.data.b
    },
    async mountAll () {
      this.monthLoad = true
