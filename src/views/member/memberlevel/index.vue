<template>
  <div class="ele-body">
    <el-card shadow="never">
        ...
        <el-row :gutter="15">
          <el-col :md="6" :sm="12">
            <el-form-item label="领域名称:">
              <el-input v-model="table.where.name" placeholder="请输入领域名称" clearable/>
            </el-form-item>
          </el-col>
          <el-col :md="6" :sm="12">
      ...
      <ele-data-table ref="table" :config="table" :choose.sync="choose" height="calc(100vh - 315px)" highlight-current-row>
        <template slot-scope="{index}">
          ...
          <el-table-column prop="name" label="领域名称" sortable="custom" show-overflow-tooltip min-width="250"/>
          ...
          <el-table-column label="操作" width="130px" align="center" :resizable="false"  fixed="right">
            <template slot-scope="{row}">
              ...
              <el-popconfirm title="确定要删除此会员领域吗？" @confirm="remove(row)" class="ele-action">
    ...
    <!-- 编辑弹窗 -->
    <el-dialog :title="editForm.id?'修改领域':'修改领域'" :visible.sync="showEdit" width="400px"
      ...
      <el-form :model="editForm" ref="editForm" :rules="editRules" label-width="82px">
        <el-form-item label="领域名称:" prop="name">
          <el-input v-model="editForm.name" placeholder="请输入领域名称" clearable/>
        </el-form-item>
  ...
  methods: {
    ...
    remove(row) {
      if (!row) {  // 批量删除
        ...
        this.$confirm('确定要删除选中的领域吗?', '提示', {type: 'warning'}).then(() => {
          const loading = this.$loading({lock: true});
