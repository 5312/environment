...
export default {
  ...
  data() {
    ...
    if (this.config.page !== false) {
      page = Object.assign(
        { page: 1, limit: 10 },
        this.config.page === true ? {} : this.config.page
      );
    }
    // 请求参数名
  ...
  methods: {
    ...
    reload(first) {
      ...
      if (
        ...
        let params,
          data = Object.assign({}, config.where || {}, pageParam, this.order);
        if (config.parseParam) {
          data = config.parseParam(data); 
        }
        ...
        if (["POST", "PUT", "PATCH"].indexOf(method) === -1) {
          ...
          config.contentType &&
          config.contentType.toLowerCase().indexOf("form") !== -1
        ) {
