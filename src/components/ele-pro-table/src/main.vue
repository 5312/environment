...
<template>
  <div
    :class="[
      ...
      { 'ele-pro-table-fullscreen': isFullscreen },
    ]"
...
export default {
  ...
  data() {
    ...
    return {
      ...
      reloading: false,
      // 表格尺寸
      tableSize: this.size || (this.$ELEMENT || {}).size,
      // 是否是全屏状态
      isFullscreen: false,
    };
  },
  ...
  watch: {
    ...
    request() {
      const globalRequest = this.$ELEADMIN ? this.$ELEADMIN.request : null;
      this.requestNames = Object.assign(
        {},
        defaultRequest,
        globalRequest,
        this.request
    ...
    reload(obj) {
      ...
      if (typeof this.datasource === "function") {
        ...
        if (["POST", "PUT", "PATCH"].indexOf(method) !== -1) {
          ...
          if (
            this.contentType &&
            this.contentType.toLowerCase().indexOf("form") !== -1
          ) {
    ...
    sortChange({ column, prop, order }) {
      if (this.$listeners["sort-change"]) {
        ...
        if (order) {
          ...
          temp[this.requestNames.orderName] = {
            ascending: "asc",
            descending: "desc",
          }[order];
        }
