    ...
    <template v-for="tool in tools">
      <template v-if="tool === 'reload'">
        <div class="ele-tool-item ele-action" :key="tool">
          <el-tooltip :content="t('ele.tableTools.refresh')" placement="top">
      ...
      <template v-if="tool === 'size'">
        <div class="ele-tool-item ele-action" :key="tool">
          <el-tooltip :content="t('ele.tableTools.size')" placement="top">
            <el-dropdown
              trigger="click"
      ...
      <template v-if="tool === 'columns'">
        <div class="ele-tool-item ele-action" :key="tool">
          <el-tooltip :content="t('ele.tableTools.columns')" placement="top">
            <el-popover
            ...
            >
              ...
              <div class="ele-tool-column-header ele-tool-column-item">
                ...
                >
                  {{ t("ele.tableTools.columnsOption.columns") }}
                </el-checkbox>
                <el-link
                  type="primary"
                  :underline="false"
                  @click="resetColumnChecked"
                >
                  {{ t("ele.tableTools.columnsOption.reset") }}
                </el-link>
              </div>
              <el-divider />
              <el-checkbox-group
                  ...
                  >
                    ...
                    <el-checkbox :label="element.key">
                      {{
                        element.title ||
                        ("index" === element.type
                          ? t("ele.tableTools.columnsOption.index")
                          : "selection" === element.type
                          ? t("ele.tableTools.columnsOption.selection")
                          : t("ele.tableTools.columnsOption.untitled"))
                      }}
                    </el-checkbox>
                  </div>
                </draggable>
      ...
      <template v-if="tool === 'fullscreen'">
        ...
        >
          <el-tooltip :content="t('ele.tableTools.fullscreen')" placement="top">
            <i
              :class="
                isFullscreen
...
export default {
  ...
  methods: {
      ...
      eachTreeData(this.orgColumns, (d) => {
        ...
        if (d.columnKey || d.prop) {
          data.push({
            ...
            title:
              d.label /* || ('index' === d.type ? '序号' : ('selection' === d.type ? '选择列' : '无标题'))*/,
            show: d.show !== false,
          });
        }
      });
    ...
    getCheckedColumns(columns, checked) {
      ...
      columns.forEach((d) => {
        if (
          this.columnList.some((c) => c.key === d.columnKey || c.key === d.prop)
        ) {
          if (
            checked.indexOf(d.columnKey) === -1 &&
            checked.indexOf(d.prop) === -1
          ) {
            return;
          }
        }
      ...
      if (this.columnsSort) {
        data.sort((a, b) => {
          let ai = this.columnList.findIndex(
              (c) => c.key === a.columnKey || c.key === a.prop
            ),
            bi = this.columnList.findIndex(
              (c) => c.key === b.columnKey || c.key === b.prop
            );
          // 父级列处理
          if (ai === -1 && a.children && a.children.length) {
            ai = this.columnList.findIndex(
              (c) =>
                c.key === a.children[0].columnKey ||
                c.key === a.children[0].prop
            );
          }
          if (bi === -1 && b.children && b.children.length) {
            bi = this.columnList.findIndex(
              (c) =>
                c.key === b.children[0].columnKey ||
                c.key === b.children[0].prop
            );
          }
          // 固定列处理
