...
export default {
    ...
    mutations: {
        SET: (state, obj) => {
            ...
            if (obj.key !== "showSetting") {
                //state.showSetting &&
                let temp = JSON.parse(
                    localStorage.getItem(setting.themeStoreName) || "{}"
                );
        ...
        setTheme({ commit, state }, theme) {
            return new Promise((resolve, reject) => {
                if (!theme) {
                    // 恢复默认主题
                    document.head
                        .querySelectorAll('style[id^="ele-theme-"],link[id^="ele-theme-"]')
                        .forEach(node => {
                    ...
                    if (oldElem) {
                        ...
                        document.head
                            .querySelectorAll(
                                'style[id^="ele-theme-"],link[id^="ele-theme-"]'
                            )
                            .forEach(node => {
                                node.parentNode.removeChild(node);
                            });
                    ...
                    document.head
                        .querySelectorAll(
                            'style:not([id^="ele-theme-"]),link:not([id^="ele-theme-"]),script:not([id^="ele-theme-"])'
                        )
                        .forEach(node => {
                            if (!node.getAttribute("ele-theme"))
                                node.setAttribute("ele-theme", "no");
                        });
                    // 加载主题css模块
                    ...
                    .then(() => {
                            ...
                            if (!elem) {
                                ...
                                elem = document.head.querySelectorAll(
                                    'link:not([ele-theme="no"])'
                                );
                                elem = elem.length ? elem[elem.length - 1] : null;
                                if (!elem) return reject(new Error("theme element not found."));
                                // 再找到对应的主题js模块的标签
                                let href = elem.getAttribute("href");
                                let uuid = href.substring(
                                    href.indexOf("chunk-"),
                                    href.indexOf(".")
                                );
                                let node = document.head.querySelectorAll(
                                    `script[src^="/js/${uuid}"]:not([ele-theme="no"])`
                                );
                                if (node.length) {
